...
def unstack_image(
    ...
    logger.info(
        f"Unstacked image data along {axis} axis with index {axis_idx}. Data shapes: {[data[i].shape for i in range(len(data))]}"
    )
    ...
    if splitting_channels:
        if "channel_names" in metadata:
            ...
            assert len(channel_names) == len(
                data
            ), "Number of channel names must match number of data arrays"
        else:
    ...
    if "scale" in metadata:
        ...
        metadata["scale"] = Scale(
            **{
                s: v if s in axes else None
                for i, (s, v) in enumerate(metadata["scale"]._asdict().items())
            }
        )
    if "dims" in metadata:
...
def squeeze_image(
    data: np.ndarray, metadata: dict[str, Any]
    ...
    if "axes" in metadata:
        axes = [
            axis for i, axis in enumerate(metadata["axes"]) if i not in singular_axes
        ]
        metadata["axes"] = axes
    else:
        axes = []
    if "scale" in metadata:
        ...
        metadata["scale"] = Scale(
            **{
                s: v if s in axes else None
                for s, v in metadata["scale"]._asdict().items()
            }
        )
...
def unsqueeze_image(
    ...
    if "scale" in metadata:
        ...
        metadata["scale"] = Scale(
            **{
                s: v if data.shape[i] != 1 else None
                for i, (s, v) in enumerate(metadata["scale"]._asdict().items())
            }
        )
...
class FileListConfig(Configuration):
    ...
    directories: bool = Field(
        default=False, description="Whether to search for directories"
    )
    files: bool = Field(default=True, description="Whether to search for files")
    recursive: bool = Field(
        default=True, description="Whether to recursively search for files"
    )
    follow_symlinks: bool = Field(
        default=True, description="Whether to follow symlinks"
    )
    ...
    skip_on_error: bool = Field(
        default=True, description="Whether to skip paths that do not exist"
    )
    ...
    def normalize_paths(cls, v: Any) -> List[Path]:
        ...
        raise ValueError(
            f"paths must be a string, Path, or list of strings/Paths, got {type(v)}"
        )
    ...
    def normalize_patterns(cls, v: Any) -> Optional[List[str]]:
        ...
        if isinstance(v, list):
            return v
        raise ValueError(
            f"include/exclude must be a string or list of strings, got {type(v)}"
        )
...
class FileList(Configurable[FileListConfig]):
    ...
    def run(self) -> list[Path]:
        ...
        for path_obj in input_paths:
            ...
            logger.debug(
                f"Processing path object: {path_obj} (is_absolute: {path_obj.is_absolute()})"
            )
            ...
            if not path_obj.exists():
                ...
                logger.error(
                    f"  Path parent exists: {path_obj.parent.exists() if path_obj.parent else False}"
                )
                ...
                if self.config.skip_on_error:
                    ...
                    logger.error(
                        f"Path does not exist: {path_obj}. Stopping file list generation."
                    )
            ...
            # If it's a directory
                        ...
                        for pattern in self.config.include:
                            ...
                            if supports_follow_symlinks:
                                ...
                                if not self.config.follow_symlinks:
                                    logger.warning(
                                        "follow_symlinks=False is not supported in Python < 3.13. Symlinks will be followed."
                                    )
                ...
                if self.config.directories and self.config.recursive:
                    ...
                    for dir_path in dir_iter:
                        if (
                            dir_path.is_dir() and dir_path != path_obj
        ...
        logger.info(
            f"Found {len(all_results)} paths matching criteria ({len([p for p in all_results if p.is_file()])} files, {len([p for p in all_results if p.is_dir()])} directories)"
        )
...
class DataWriterConfig(Configuration):
    ...
    path: Union[str, Path] = Field(
        default=".", description="Path to output file or directory"
    )
...
class DataWriter(Configurable[DataWriterConfig]):
    ...
    def run(
        self, data: Any, path: Path | str, metadata: Optional[dict[str, Any]] = None
    ) -> None:
...
class DataFrameWriterConfig(DataWriterConfig):
    ...
    save_index: bool = Field(
        default=True, description="Whether to save the index of the dataframe"
    )
...
class ImageWriterConfig(DataWriterConfig):
    ...
    writer: Optional[Writer] = Field(default=None, description="OME-TIFF writer")
...
class ImageWriter(DataWriter):
    ...
    def run(
        ...
        try:
            ...
            logger.error("OmeTiffWriter not installed.")
        # Expand ~/ at runtime (not in validator) to preserve portability across users
        if isinstance(path, str):
            path = Path(path).expanduser()
        else:
...
class ImageLoaderConfig(DataLoaderConfig):
    ...
    grayscale: bool = Field(
        default=False, description="Whether to convert loaded images to grayscale"
    )
    split_channels: bool = Field(
        default=False,
        description="Whether to split channels into separate files after processing",
...
class ImageLoader(DataLoader):
    ...
    def _load_image(
        ...
        try:
            ...
            raise ImportError(
                "bioio is required for load_image. Install it with: pip install bioio"
            )
        ...
        logger.info(f"Scenes found: {reader_instance.scenes}")
        if scene_index is not None:
            ...
            if scene_index < 0 or scene_index >= num_scenes:
                raise ValueError(
                    f"Scene index {scene_index} is out of range (0-{num_scenes-1})"
                )
        ...
        if substack is not None and len(substack) > 0:
            logger.info(f"Applying substack={substack}")
            # reader_instance.set_scene(scene_index)
            ...
            if None in substack:
                ...
                first_dim = (
                    reader_instance.dims.order[0].upper()
                    if reader_instance.dims.order
                    else "T"
                )
                ...
                logger.debug(
                    f"Legacy substack format: applied to first dimension '{first_dim}'"
                )
        ...
        if rename_channel:
            renamed_ch_names = [
                rename_channel[ch_name] if ch_name in rename_channel.keys() else ch_name
