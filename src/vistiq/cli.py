...
def parse_substack(substack: Optional[str]) -> tuple[Optional[int], Optional[int]]:
    ...
    if "-" in fs:
        ...
        if not a.isdigit() or not b.isdigit():
            raise ValueError("--substack must be positive integers like '10' or '2-40'")
        a_i = int(a)
        b_i = int(b)
        if a_i < 1 or b_i < 1:
...
class CLISubcommandConfig(CLIAppConfig):
    ...
    input: Optional[FileListConfig] = Field(
        default=None,
        description="Configuration for collecting input file(s) or directories",
    )
    loader: Optional[DataLoaderConfig] = Field(
        default_factory=DataLoaderConfig,
        description="Configuration to specify the data loader to use",
    )
    ...
    @classmethod
    def validate_step(
        cls, v: Optional[list[Configuration]]
        ...
        for item in v:
            if not isinstance(item, Configuration):
                raise ValueError(
                    f"Each item in 'step' must be a Configuration instance, got {type(item)}"
                )
        return v
...
class CLIPreprocessorConfig(CLISubcommandConfig):
    ...
    output: Optional[ImageWriterConfig] = Field(
        default_factory=ImageWriterConfig,
        description="Configuration for output data writing",
    )

    @field_validator("step")
    @classmethod
    def validate_step(
        cls, v: Optional[list[PreprocessorConfig]]
    ) -> Optional[list[PreprocessorConfig]]:
        ...
        for item in v:
            if not isinstance(item, PreprocessorConfig):
                raise ValueError(
                    f"Each item in 'step' must be a PreprocessorConfig instance, got {type(item)}"
                )
        return v
...
class CLISegmenterConfig(CLISubcommandConfig):
    ...
    checkpoint: Optional[Path] = Field(
        default=None,
        description="Optional path to a .pth model checkpoint to load for segmentation",
    )
...
class CLITrainerConfig(CLISubcommandConfig):
    ...
    dataset: Optional[DatasetCreatorConfig] = Field(
        default_factory=DatasetCreatorConfig,
        description="Configuration to specify the dataset creator to split the data into training and validation sets",
    )
    step: Optional[list[Configuration]] = Field(
    ...
    min_object_size: int = Field(
        default=50,
        description="Minimum object size (in pixels) used during fine-tuning",
    )

    @field_validator("step")
    @classmethod
    def validate_step(
        cls, v: Optional[List[TrainerConfig]]
    ) -> Optional[List[TrainerConfig]]:
        """Validate that steps are valid.
...
def substack_to_slices(substack: Optional[str]) -> Optional[dict[str, slice]]:
    ...
    if ":" in substack:
        ...
        def parse_dimension_range(range_str: str) -> tuple[int, int]:
            ...
            if "-" in range_str:
                ...
                if len(parts) != 2 or not parts[0].isdigit() or not parts[1].isdigit():
                    raise ValueError(
                        f"Invalid range format: '{range_str}'. Expected 'A-B' or 'A'"
                    )
                start = int(parts[0])
                end = int(parts[1])
                if start < 1 or end < 1:
        ...
        for part in parts:
            if ":" not in part:
                raise ValueError(
                    f"Invalid substack format: '{part}'. Expected 'DIM:RANGE'"
                )
        ...
        for i, component_config in enumerate(component_configs):
            ...
            if configurable_class:
                try:
                    ...
                    logger.info(
                        f"Built component {i+1}/{len(component_configs)}: {configurable_class.__name__}"
                    )
                ...
                logger.warning(
                    f"Could not find configurable class for config type {config_class.__name__}"
                )
...
def common_callback(
    ...
    logger.debug(
        f"common_callback: Set context - loglevel={loglevel.upper()}, device={device}, processes={processes}"
    )
...
def cli_to_config(
    value: str, default_value_field: str = "classname", alt_classname: str = None
) -> Configuration:
    ...
    logger.debug(
        f"Final config_dict={config_dict}, config_class_name={config_class_name}"
    )
    ...
    logger.debug(
        f"Looking up config_class_name='{config_class_name}', found: {config_class}"
    )
    ...
    if config_class is None:
        ...
        logger.debug(
            f"Looking up configurable_class_name='{config_class_name}', found: {configurable_class}"
        )
        if configurable_class is not None:
            ...
            logger.debug(
                f"Looking up derived config '{derived_config_name}', found: {config_class}"
            )
    ...
    try:
        ...
        raise ValueError(
            f"Failed to create configuration for '{config_class_name}': {e}"
        ) from e
...
def cli_to_filelist_config(value: str) -> FileListConfig:
    ...
    """
    return cli_to_config(
        value, default_value_field="paths", alt_classname="FileListConfig"
    )
...
def cli_to_imageloader_config(value: str) -> ImageLoaderConfig:
    ...
    return cli_to_config(
        value, default_value_field="classname", alt_classname="ImageLoaderConfig"
    )
...
def cli_to_imagewriter_config(value: str) -> ImageWriterConfig:
    ...
    """
    return cli_to_config(
        value, default_value_field="path", alt_classname="ImageWriterConfig"
    )
...
def preprocess_cmd(
    ...
    input: Optional[FileListConfig] = Option(
        ...
        "--input",
        "-i",
        help="Input file or directory configuration",
    ...
    step: List[PreprocessorConfig] = Option(
        None,
        "--step",
        "-s",
        help="Processing step/component to include (can be specified multiple times). Use --step NAME to add a step.",
    ...
    output: Optional[ImageWriterConfig] = Option(
        ...
        "--output",
        "-o",
        help="Output file or directory configuration",
...
def segment_cmd(
    ...
    checkpoint: Optional[Path] = Option(
        ...
        "--checkpoint",
        "-c",
        help="Path to a model checkpoint (.pth) to load",
    ),
) -> None:
...
def train_cmd(
    ...
    labels: Optional[FileListConfig] = Option(
        None,
        "--labels",
        "-l",
        help="Input file or directory configuration",
    ...
    dataset: Optional[DatasetCreatorConfig] = Option(
        None,
        "--dataset-creator",
        "-d",
        help="Configuration to specify the dataset creator to use",
        parser=cli_to_component_config,
    ),
    loader: Optional[ImageLoaderConfig] = Option(
        None,
    ...
    step: List[TrainerConfig] = Option(
        None,
        "--step",
        "-s",
        help="Processing step/component to include (can be specified multiple times). Use --step NAME to add a step.",
    ...
    checkpoint: Optional[Path] = Option(
        None, "--checkpoint", "-c", help="Path to a model checkpoint (.pth) to load"
    ),
    pred_iou_thresh: float = Option(
        0.88,
        "--pred-iou-thresh",
        help="Prediction IoU threshold used during fine-tuning",
    ),
    stability_score_thresh: float = Option(
        0.95,
        "--stability-score-thresh",
        help="Stability score threshold used during fine-tuning",
    ),
    box_nms_thresh: float = Option(
        0.7, "--box-nms-thresh", help="Box NMS threshold used during fine-tuning"
    ),
    min_object_size: int = Option(
        50,
        "--min-object-size",
        help="Minimum object size (in pixels) used during fine-tuning",
    ),
) -> None:
    ...
    patterns = (
        config.input.include[0] if config.input.include is not None else None,
        config.labels.include[0] if config.labels.include is not None else None,
    )
    dc = config.dataset.copy(
        update={"patterns": patterns, "out_path": config.output.path}
    )
...
def run_preprocess(config: CLIPreprocessorConfig) -> None:
    ...
    if isinstance(file_list_result, list):
        ...
        file_list = (
            ...
            if hasattr(file_list_result, "__iter__")
...
def run_training(config: CLITrainerConfig) -> None:
    ...
    if isinstance(input_list_result, list):
        ...
        # If Prefect wrapped it somehow, try to unwrap
        input_list = (
            list(input_list_result)
            if hasattr(input_list_result, "__iter__")
            else [input_list_result]
        )
    if not input_list:
    ...
    if isinstance(labels_list_result, list):
        ...
        labels_list = (
            list(labels_list_result)
            if hasattr(labels_list_result, "__iter__")
            else [labels_list_result]
        )
    if not labels_list:
        logger.error(f"No files found for input path: {config.input.paths}")
        return
    ...
    if config.checkpoint is not None:
        ...
        if current in (None, "", False):
            microsam_trainer_config = microsam_trainer_config.copy(
